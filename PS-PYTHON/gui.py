...
root.title("EXPENSE TRACKER BY ANANT SINGH (66), KINJAL SINGH (68), ARYA TIWARI (69)")
root['background']='#f79b83'
...
def note_clear(event):
    ...
    if new_window_counter == 0:
        ...
        text = Text(new_window, font=("Times New Roman", 11), width=50, height=20)
        text.insert(END, note_entry.get())
        text.pack(padx=20, pady=20)

        ok_button = Button(new_window, font=("Times New Roman", 12), text="Done", command=lambda: (note_entry.delete(0, END),
...
def add_data(*args):
    ...
    values = db.db_show(month_combo2.get(), year_combo.get())
    for row in values:
    ...
    if tree.selection():
        ...
        if question == "yes":
            for record in tree.selection():
                db.delete(tree.item(record)['values'][6])
                tree.delete(record)
...
def modify_window():
    ...
    if new_window2_counter == 0:
        try:
            ...
            "creating labels"
            value_label = Label(new_window2, text="Value", font=("Times New Roman", 10))
            value_label.grid(row=0, column=0)

            category_label = Label(new_window2, text="Category", font=("Times New Roman", 10))
            category_label.grid(row=0, column=1)

            note_label = Label(new_window2, text="Note", font=("Times New Roman", 10))
            note_label.grid(row=2, column=0, columnspan=2, pady=(20, 0))

            "creating entries"
            value_entry2 = Entry(new_window2, font=("Times New Roman", 10), width=10)
            value_entry2.insert(0, tree.item(selection)['values'][3])
            value_entry2.grid(row=1, column=0, padx=10, sticky=N)

            category_entry2 = Entry(new_window2, font=("Times New Roman", 10), width=20)
            category_entry2.insert(0, tree.item(selection)['values'][4])
            category_entry2.grid(row=1, column=1, padx=10, sticky=N)

            note_entry2 = Text(new_window2, font=("Times New Roman", 11), width=50, height=20)
            note_entry2.insert(END, tree.item(selection)['values'][5])
            note_entry2.grid(row=3, column=0, columnspan=2, padx=20, pady=(10, 20))

            modify_button2 = Button(new_window2, text="Modify record", font=("Times New Roman", 12),
...
def expense_details_f():
    ...
    query_result = db.statistics(month_combo2.get(), year_combo.get())

    tittle = "Expense details: {} {}".format(month_combo2.get(), year_combo.get())
    expense_details = Label(frame3, font=("Times New Roman", 14), text=tittle)
    ...
    try:
        ...
        total_amount = Label(frame3, font=("Times New Roman", 11), text=footer)
    ...
    for row in query_result:
        ...
        x = Label(frame3, font=("Times New Roman", 11), text=label_text)
...
# creating labels
#   frame 1
insert_label = Label(frame1, text="Insert:", font=("Times New Roman", 14))
insert_label.grid(row=0, column=0, padx=10, pady=(10, 0), sticky=W)

year_label = Label(frame1, text="Year  ", font=("Times New Roman", 14))
year_label.grid(row=0, column=4, sticky=E, pady=(10, 0))

#   frame 2
records_label = Label(frame2, text="Records", font=("Times New Roman", 14)).grid(row=0, column=0, columnspan=3)
note_view_label = Label(frame2, justify=LEFT, wraplength=400, font=("Times New Roman", 11))
...
month_combo = ttk.Combobox(frame1, value=months, font=("Times New Roman", 10), state="readonly", width=12)
month_combo.current(month)
month_combo.bind("<<ComboboxSelected>>", update_day)
month_combo.grid(row=1, column=0, padx=10)

# year
year_combo = ttk.Combobox(frame1, value=[2020, 2021, 2022], font=("Times New Roman", 10), state="readonly", width=5)
year_combo.set(year)
...
day_combo = ttk.Combobox(frame1, value=day_selector_1(month_combo.get(), year_combo.get()),
                         font=("Times New Roman", 10), state="readonly", width=10)
day_combo.current(today)
day_combo.grid(row=1, column=1, padx=10)

# frame2
month_combo2 = ttk.Combobox(frame2, value=months, font=("Times New Roman", 10), state="readonly", width=12)
...
# creating entries
# frame 1
value_entry = Entry(frame1, font=("Times New Roman", 10), width=10)
value_entry.insert(0, 'Enter value')
...
category_entry = Entry(frame1, font=("Times New Roman", 10), width=20)
category_entry.insert(0, "Enter expense category")
...
note_entry = Entry(frame1, font=("Times New Roman", 10))
note_entry.insert(0, "Enter note (optional)")
...
# creating buttons
insert_button = Button(frame1, text="Insert", font=("Times New Roman", 12), command=lambda: (insert_db(), add_data()))
insert_button.grid(row=1, column=5, padx=10, pady=10)

delete_button = Button(frame2, text="Delete record", font=("Times New Roman", 12), command=delete_record)
delete_button.grid(column=0, row=3, pady=10)

modify_button = Button(frame2, text="Update record", font=("Times New Roman", 12), command=modify_window)
modify_button.grid(column=1, row=3, pady=10)

see_note_button = Button(frame2, text="See note", font=("Times New Roman", 12), command=see_note)
see_note_button.grid(column=2, row=3, padx=(0, 10))

hide_note_visualization = Button(frame2, text="Hide", font=("Times New Roman", 12), command=lambda: (note_view_label.grid_remove(),
